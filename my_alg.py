...
def build_model_128(): # 建立128*128模型  ##### 不是很懂为什么这几行重复的代码就完成了模型的建立，我再研究研究
    acf = tf.nn.relu # 线性整流函数，将负数变为0
    img_input = keras.Input(shape=(128, 128, 1)) # 初始化网络输入层
    # out 32 x 32 @

    block_0_in = keras.layers.Conv2D(32, 3, padding='same', activation=acf)(img_input) # 描述卷积层
    block_0 = keras.layers.BatchNormalization()(block_0_in) # 使数据接近0，标准差接近1

    block_0 = keras.layers.Conv2D(32, 3, padding='same', activation=acf)(block_0_in)
    block_0 = keras.layers.Conv2D(32, 3, padding='same', activation=acf)(block_0)
    block_0 = keras.layers.BatchNormalization( )(block_0)
    block_0 = keras.layers.add([block_0, block_0_in]) # 对张量执行求和运算
    ...
    # skip connestion
    up_block_0 = keras.layers.concatenate([block_2, up_block_0]) # 对张量进行串联运算
